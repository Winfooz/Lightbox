...
public protocol LightboxControllerPageDelegate: class {
	...
	func lightboxControllerWillDelete(_ controller: LightboxController,  page: Int)
}
...
open class LightboxController: UIViewController {
	...
	func reconfigurePagesForPreload() {
		...
		for i in 0..<initialImages.count {
			if let pageView = pageViews[safe: i] {
...
extension LightboxController: HeaderViewDelegate {
	...
	func headerView(_ headerView: HeaderView, didPressDeleteButton deleteButton: UIButton) {
		...
		guard numberOfPages != 1 else {
			...
			self.pageDelegate?.lightboxControllerWillDelete(self, page: 0)
			return
		}
		...
		self.pageDelegate?.lightboxControllerWillDelete(self, page: prevIndex)
